            ...
            <?php
                if(!empty($avail_labels))
                {
                    for($i=0;$i<count($avail_labels);$i++)
                    {
                        ...
                        {
                            echo '<span class="glyphicon glyphicon-trash ticket_label_delete_type" id="'.$avail_labels[$i]['slug'].'" data-toggle="wsdesk_tooltip" data-container="body" title="Delete Status" style="margin-right:5px;cursor:pointer;font-size: large;"></span> ';
                        }
                        echo '<span class="glyphicon glyphicon-pencil ticket_label_edit_type" id="'.$avail_labels[$i]['slug'].'" data-toggle="wsdesk_tooltip" data-container="body" title="Edit Status" style="margin-right:5px;cursor:pointer;font-size: large;"></span> ';
                        echo '</span></li>';
                    }
                }
                else
                {
                    echo '<li class="list-group-item list-group-item-info">There are no Labels! Create One Label.</li>';
                }
            ?>
        </ul>
    </div>
        ...
        <div class="crm-form-element">
            <div class="col-md-12">
                ...
                <span style="vertical-align: middle;" id="ticket_label_add_section">
                    ...
                    <span class="help-block"><?php _e('Do you want to change the status color?.', 'wsdesk'); ?></span>
                    <span style="vertical-align: middle;">
                        <input type="color" id="ticket_label_add_color"/><span> <?php _e('Click and Pick the Color', 'wsdesk'); ?></span>
                    </span>
                    <span class="help-block"><?php _e('Want to use this status for filter tickets?', 'wsdesk'); ?> </span>
                    <input type="radio" style="margin-top: 0;" checked id="ticket_label_add_filter" class="form-control" name="ticket_label_add_filter" value="yes"> <?php _e('Yes! I will use it for Filter', 'wsdesk'); ?><br>
