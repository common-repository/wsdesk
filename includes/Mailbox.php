...
class EH_CRM_Mailbox {
	 ...
	 */
	public function searchMailbox($criteria = 'ALL',$charset='UTF-8') {
		$mailsIds = imap_search($this->getImapStream(), $criteria, SE_UID, $charset);
	...
	public function getMail($mailId, $markAsSeen = true) {
		...
		$mail = new EH_CRM_IncomingMail();
		$mail->headersRaw = $headersRaw;
		$mail->headers = $head;
		$mail->id = $mailId;
		$mail->date = date('d-M-Y h:i:s A', isset($head->date) ? strtotime(preg_replace('/\(.*?\)/', '', $head->date)) : time());
	...
	protected function initMailPart(EH_CRM_IncomingMail $mail, $partStructure, $partNum, $markAsSeen = true) {
        $options = FT_UID;
        if(!$markAsSeen) {
            $options |= FT_PEEK;
        }
		...
		if($attachmentId) {
			...
			$ext = pathinfo($fileName, PATHINFO_EXTENSION);
                        if (!in_array($ext, array('php', 'exe', 'sh', 'js'))) {
                            $attachment = new EH_CRM_IncomingMailAttachment();
                            $attachment->id = $attachmentId;
                            $attachment->name = time() . '_' . $fileName;
                            $attachment->disposition = (isset($partStructure->disposition) ? $partStructure->disposition : null);
                            if($this->attachmentsDir) {
                                    $fileSysName = time() . '_' . $fileName;
